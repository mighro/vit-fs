...
def test_vit_embeddings_patch_dropout():
    """Test that patch dropout actually zeros out some tokens."""
    embed_dim = 768
    patch_size = 16
    image_size = (224, 224)
    ...
    # With 50% dropout, roughly half of the non-CLS tokens should be zero
    patch_tokens = out[:, 1:, :]  # Exclude CLS token
    zero_ratio = (patch_tokens == 0).float().mean()
    assert 0.4 < zero_ratio < 0.6  # Allow some variance due to randomness
