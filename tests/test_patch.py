    ...
    for img_size in [(32, 32), (64, 64), (128, 128)]:
        for p_size in [8, 16, 32]:
            if img_size[0] % p_size == 0 and img_size[1] % p_size == 0:
                pe = PatchEmbedding(in_channels=3, patch_size=p_size, image_size=img_size, embed_dim=512)
...
def test_patch_embedding_grayscale():
    ...
    pe = PatchEmbedding(in_channels=1, patch_size=16, image_size=(224, 224), embed_dim=768)
...
def test_patch_embedding_invalid_dimensions():
    ...
    pe = PatchEmbedding(in_channels=3, patch_size=16, image_size=(224, 224), embed_dim=768)
